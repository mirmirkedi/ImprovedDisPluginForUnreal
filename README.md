...
This repository is an independently published derivative of [AF-GRILL/DISPluginForUnreal](https://github.com/AF-GRILL/DISPluginForUnreal). It keeps the original GRILL DIS for Unreal foundation, OpenDIS integration, and BSD-2-Clause license while documenting the fixes and compatibility updates added for current Unreal Engine workflows.

## Fork Highlights

- Updated the plugin descriptor for Unreal Engine 5.7.
- Added explicit Win64 and macOS platform support in both the `.uplugin` module allow lists and `DISRuntime.Build.cs`.
- Added Blueprint platform helpers for runtime platform checks: `GetCurrentPlatformName`, `IsRunningOnWindows`, and `IsRunningOnMac`.
- Preserved and documented network quality-of-life improvements including receive loopback control, receive-on-game-thread handling, and heartbeat-based Entity State / Entity State Update transmission.
- Keeps the original GeoReferencing-based DIS workflow and content support while making the fork easier to reuse in modern UE5 projects.

# Getting Started

- This fork is currently set up for Unreal Engine 5.7 on Win64 and macOS
...
In accordance with the BSD-2-Clause license, this repository keeps the original license text in [LICENSE](LICENSE). If you redistribute this repository in source or binary form, retain the original copyright notice, license terms, and disclaimer.
